                ...
                <div key={index}>
                    ...
                    <div className="content">
                        ...
                        <div className="content-label">
                            ...
                            <p> {checkroom(item.room_name) >= 0 ? `ห้องที่สามารถจองได้ : ${item.number_of_rooms - checkroom(item.room_name)}` : `ห้องที่สามารถจองได้ : ${item.number_of_rooms}`} </p>
                        ...
                        <div>
                                ...
                                (numcat >= item.number_of_cats * item.number_of_rooms - checkroom(item.room_name) || numcat >= item.number_of_cats * item.number_of_rooms)
                                ?  <button className="btn-primary">{"จำนวนกล้องไม่เพียงพอและ "  + `ต้องการ ${Math.ceil((numcat) / item.number_of_cats)} ห้อง แต่เหลือเพียง  ${item.number_of_rooms - checkroom(item.room_name) ?  item.number_of_rooms - checkroom(item.room_name) : item.number_of_rooms} ห้องว่าง `}</button>
                                : numcat >= item.number_of_cats * item.number_of_rooms - checkroom(item.room_name) || numcat > item.number_of_cats * item.number_of_rooms
                                ? <button className="btn-primary">{` ต้องการ ${Math.ceil((numcat) / item.number_of_cats)} ห้อง แต่เหลือเพียง ${item.number_of_rooms - checkroom(item.room_name) ?  item.number_of_rooms - checkroom(item.room_name) : item.number_of_rooms} ห้องว่าง `}</button>
                                : numcamera > item.cameras * (Math.ceil((numcat) / item.number_of_cats)) 
                                ? <button className="btn-primary">จำนวนกล้องไม่เพียงพอ</button>
                                :  <Link to={`/detail/${item.type}`}><button className="btn-primary" onClick={() => { saveToLocalStorage(index) }}>จองที่พัก</button> </Link>
                                } 
